...
for (const dir of dirsToSync) {
  ...
  if (fs.existsSync(targetDir)) {
    try {
      ...
      console.warn(`Warning: Could not remove existing directory ${targetDir}: ${e.message}`);
    }
  }
  ...
  try {
    ...
    console.log(`  Successfully symlinked ${dir} directory (${type}).`);
  } catch (err) {
    console.warn(`  Failed to create symlink for ${dir}: ${err.message}. Falling back to copying files...`);
    // Fallback to copy
    copyFolderRecursiveSync(sourceDir, targetDir);
    console.log(`  Successfully copied ${dir} directory.`);
  }
}
